...
template <class T, int N, template <class, int> class ImplF, bool UseMask, bool UseProperties>
bool test(queue q) {
...
template <class T, int N, class C, C Op> struct ImplIncBase {
  ...
  static T gold(int i, bool use_mask) {
    T gold =
        is_updated(i, N, use_mask) ? (T)(repeat * threads_per_group * n_groups) : init(i);
    return gold;
  }
};
...
struct ImplLSCFcmpwr
    : ImplCmpxchgBase<T, N, atomic_op, atomic_op::fcmpxchg> {};
...
bool test_int_types(queue q) {
  ...
  if constexpr (SignMask & Signed) {
    ...
    if constexpr (std::is_same_v<Op<int64_t, N>, ImplCmpxchg<int64_t, N>>) {
      passed &= test<int64_t, N, Op, UseMask, UsePVCFeatures>(q);
    }
  }
  ...
  if constexpr (SignMask & Unsigned) {
    ...
    if constexpr (std::is_same_v<Op<uint64_t, N>, ImplCmpxchg<uint64_t, N>>) {
      passed &= test<uint64_t, N, Op, UseMask, UsePVCFeatures>(q);
    }
  }
  return passed;
}
...
template <bool UseMask, bool UsePVCFeatures>
